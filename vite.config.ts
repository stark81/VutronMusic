...
import { resolve, dirname, posix, join } from 'path'
...
function bindingSqlite3(
  ...
  return {
    ...
    config(config) {
      ...
      const betterSqlite3Node = posix.join(
        betterSqlite3Root,
        'build/Release',
        options.better_sqlite3_node as string
      )
      const betterSqlite3Copy = posix.join(output, options.better_sqlite3_node as string)
      ...
      const BETTER_SQLITE3_BINDING = betterSqlite3Copy.replace(resolvedRoot + '/', '')
      fs.writeFileSync(
