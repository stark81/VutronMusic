...
**文件**: `src/renderer/store/state.ts`（188 行）
**依赖**: 无
...
**模态框状态**（均为 `ref<boolean>` 或 `reactive`）:
`setConvolverModal`, `setPlaybackRateModal`, `setPitchModal`, `setThemeModal`, `setFontModal`, `setSaveThemeModal`, `selectDirModal`, `addTrackToPlaylistModal`, `newPlaylistModal`, `accurateMatchModal`, `backgroundModal`, `editPlaylistModal`
...
**文件**: `src/renderer/store/settings.ts`（328 行）
**依赖**: 无
**持久化**: 全量持久化（`omit: ['playerThemeNew']`）
...
**文件**: `src/renderer/store/player.ts`（1309 行）
**依赖**: `audioEngine`, `lyric`, `pluginMusic`, `osdLyric`, `state`, `settings`
**持久化**: 排除 `pic`, `title`, `fmTracks`
...
**IPC 监听**（通过 `handleIpcRenderer` 注册）:
`play`, `previous`, `next`, `repeat`, `repeat-shuffle`, `like`, `fm-trash`, `setPosition`, `increaseVolume`, `decreaseVolume`, `resume`
...
**文件**: `src/renderer/store/audioEngine.ts`（730 行）
**依赖**: `settings`
...
**文件**: `src/renderer/store/lyric.ts`（195 行）
**依赖**: `state`, `settings`, `osdLyric`
**持久化**: 全量
...
**文件**: `src/renderer/store/pluginMusic.ts`（946 行）
**依赖**: `state`
**持久化**: `services`, `additionalTags`, `users`, `tools`, `scanDir`, `enableLibrary/Stream/Local`
...
**文件**: `src/renderer/store/playerTheme.ts`（252 行）
**依赖**: 无
**持久化**: 全量
...
**文件**: `src/renderer/store/osdLyric.ts`（67 行）
**依赖**: 无
**持久化**: 全量
...
**文件**: `src/renderer/store/data.ts`（239 行）
**依赖**: `state`
**持久化**: `user`, `likedSongPlaylistID`, `lastRefreshCookieDate`, `loginMode`
...
```typescript
// 只持久化部分字段
{ persist: { pick: ['user', 'likedSongPlaylistID', 'lastRefreshCookieDate'] } }

// 持久化全部（排除某些字段）
{ persist: { omit: ['pic', 'title'] } }
```
...
export const usePlayerStore = defineStore('player', () => {
  ...
  function playOrPause() { audioEngine.play(); lyric.updateIndex() }
  return { playOrPause }
})
```
