...
ipcMain.on('playDiscordPresence', (event, track: NewTrack) => {
  client.details(track.name)
  client.state(track.artists[0]?.name || '未知歌手')
  client.start(Date.now() - seekTime * 1000)
  client.largeImageKey('cover')
  client.largeImageText(track.album?.name || '')
})
