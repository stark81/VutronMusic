<template>
  <div class="play-page" :class="{ 'no-lyric': noLyric && show === 'lyric' }" :data-theme="theme">
    ...
    <div class="lyrics-container" :style="{ '--bg-blur': `${backgroundBlur}px`, '--bg-opacity': backgroundOpacity / 100, '--lyric-font': fontFamily || 'inherit' }">
      <!-- 自定义背景 -->
...
const { nTranslationMode, textAlign, fontFamily, backgroundType, backgroundSource, backgroundBlur, backgroundOpacity, apiRefreshMode, apiRefreshInterval } = toRefs(normalLyric.value)
...
const loadCustomBackground = async () => {
  ...
  if (backgroundType.value === 'folder') {
    // 文件夹随机
    if (backgroundSource.value) {
      const files = await window.mainApi?.invoke('getFilesInFolder', backgroundSource.value, ['png', 'jpg', 'jpeg', 'webp', 'gif', 'mp4', 'webm'])
...
// 背景类型或来源变化时更新
watch([backgroundType, backgroundSource], () => {
  if (backgroundType.value === 'default') {
    customBgUrl.value = ''
    stopApiRefreshTimer()
  } else {
