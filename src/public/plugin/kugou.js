...
apis.db
  ...
  .catch((e) => console.debug('[kugou] 读取 store 配置失败:', e?.message))

const youthVips = [
  () => get('youth/day/vip', { receive_day: _formatDate() }),
  () =>
    setTimeout(() => {
      get('youth/day/vip/upgrade').catch((e) =>
        console.debug('[kugou] youthVip 升级失败:', e?.message)
      )
    }, 10 * 1000)
]
...
const updateVip = () => {
  return get('user/vip/detail')
    .then((result) => {
      ...
      result.data?.busi_vip.forEach(async (item, idx) => {
        if (!item.is_vip) {
          try {
            await youthVips[idx]()
          } catch (e) {
            console.error('[youthVips error]', e) // 或静默处理
          }
        }
      })
    })
    .catch((e) => console.debug('[kugou] 获取 VIP 详情失败:', e?.message))
}
...
apis.store
  ...
  .then(async (store) => {
    ...
    await get('register/dev')
      ...
      .catch((e) => console.debug('[kugou] register/dev 失败:', e?.message))
    ...
    get('artist/lists', { type: 0, hotsize: 30 })
      ...
      .catch((e) => console.debug('[kugou] 获取关注歌手列表失败:', e?.message))
    ...
    get('user/follow')
      ...
      .catch((e) => console.debug('[kugou] 获取关注用户失败:', e?.message))
    ...
    updateVip()
      .then(() => {
        get('user/playlist', { pagesize: 100 })
          ...
          .catch((e) => console.debug('[kugou] 获取用户歌单失败:', e?.message))
      })
      .catch((e) => console.debug('[kugou] updateVip 链路失败:', e?.message))

    try {
      if (!user.token) return
      get('login/token', { userid: user.userId, token: user.token })
        .then((res) => {
          user.token = res.data?.token || user.token
          apis.db.set('PluginData', user)
        })
        .catch((e) => console.debug('[kugou] 刷新 login token 失败:', e?.message))
    } catch (error) {
      console.log('[kugou.js update token error]: ', error)
    }
  })
  .catch((e) => console.debug('[kugou] 读取 store 配置失败:', e?.message))
  ...
  try {
    ...
    const album = {
      ...
      picUrl: (
        pick(
          ...
          item.Image,
          item.relate_goods?.[0]?.info?.image,
    ...
    const artists = pick(
      ...
      [
        {
          id: artistId || 0,
          name: item.author_name || item.singername,
          picUrl: '',
          pluginId: '',
    ...
    return {
      ...
      duration:
        ...
        item.timelength ??
        item.duration ??
        (item.time_length ?? item.Duration ?? 0) * 1000,
      alias: [],
      createTime:
        parseDate(
...
const formatPlaylistDetail = (playlist) => {
  ...
  return {
    ...
    subscribed: idx !== -1,
    picUrl:
      (playlist.pic || '').replace('{size}', '512') ||
      'https://c1.kgimg.com/stdmusic/aaa/ddd/ddd.jpg',
    trackCount: playlist.count || 0,
    updateTime: new Date((playlist.update_time || 0) * 1000).getTime(),
...
/**
 * 评分匹配：与传入的原始歌曲元数据比对候选歌曲
 * @param {{ name: string, album?: string, artists: string[], duration: number }} origin
 * @param {{ songname?: string, album_name?: string, author_name?: string, timelen?: number }} candidate
 * @returns {number} 0-100
 */
function scoreMatch(origin, candidate) {
  let score = 0

  // 歌曲名完全一致（归一化后）
  if (normalize(origin.name) === normalize(candidate.OriSongName)) score += 40

  // 专辑名完全一致（归一化后）
  if (origin.album && normalize(origin.album) === normalize(candidate.AlbumName)) score += 20

  // 至少一个歌手匹配（SingerName 可能是 "国风集、陆深" 这种合并字符串）
  if (
    candidate.SingerName &&
    origin.artists.some((oa) => normalize(candidate.SingerName).includes(normalize(oa)))
  )
    score += 20

  // duration 误差 ±3s（origin.duration 是 ms，candidate.Duration 是 s）
  if (Math.abs(Math.round(origin.duration / 1000) - candidate.Duration) <= 3) score += 20
...
const meta = {
  ...
  capabilities: {
    ...
    mv: { detail: true, like: true, subscribe: false }
  }
}
...
const buildRankTrackInfo = (item) => ({
  ...
  albumArtists: item.authors.map((ar) => ({
    id: ar.author_id ?? '',
    name: ar.author_name ?? '',
    picUrl: ar.sizable_avatar?.replace('{size}', '512') || '',
    pluginId: '',
 ...
 * 歌曲精确匹配
 *
 * 酷狗没有官方 match API，使用 search 搜索候选后通过 scoreMatch 评分，
...
exports.scrobble = async (params) => {
  try {
    ...
    await get('playhistory/upload', { mxid: params.mxid || params.album_audio_id, time })
    return { code: 200 }
  } catch (error) {
...
const getTrackComments = async (params) => {
  try {
    const { mxid, album_audio_id: aai, page = 1 } = params
    const mixsongid = mxid ?? aai
...
exports.fmTrash = async (params) => {
  // params is track.sourceContext = { id, hash, name, album, artists, picUrl, fileid, mxid }
  try {
